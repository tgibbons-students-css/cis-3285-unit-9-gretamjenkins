...
namespace SingleResponsibilityPrinciple
{
    public class SimpleTradeValidator : ITradeValidator
    {
        ...
        public bool Validate(string[] tradeData)
        {
            ...
            {
                logger.LogWarning("WARN", " Trade amount on line {0} is greater than 10000: '{1}'", tradeData[1]);
                return false;
            }
            if (tradeAmount < 1000)
            {
                logger.LogWarning("WARN", " Trade amount on line {0} is less than 1000: '{1}'", tradeData[1]);
                return false;
            }
